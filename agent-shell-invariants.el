...
Each event is a small plist; 5000 entries uses roughly 1-2 MB.")
...
(defun agent-shell-invariants--check-fragment-ordering ()
  "Verify fragment buffer positions are monotonically increasing per namespace.
Within a namespace, each successive fragment must appear at a higher
buffer position than the previous one.  A decrease indicates that a
fragment was inserted above an earlier sibling, which would happen if
the insert-cursor regressed.  Note: this checks buffer position order,
not creation order — it cannot detect creation-order bugs when positions
    ...
    (save-excursion
      ...
      (let ((match t))
        (while (setq match (text-property-search-forward
                            'agent-shell-ui-state nil
                            (lambda (_ v) v) t))
          (let* ((state (prop-match-value match))
                 (qid (map-elt state :qualified-id))
                 (pos (prop-match-beginning match)))
            (when qid
              ;; Deduplicate: only record first occurrence of each qid
              (unless (assoc qid fragments)
                (push (cons qid pos) fragments)))))))
    ;; fragments is in buffer order (reversed because of push)
    (setq fragments (nreverse fragments))
    ;; Group by namespace and check ordering within each group
    (let ((by-ns (make-hash-table :test 'equal))
          (violations nil))
      (dolist (entry fragments)
        (let* ((qid (car entry))
               (pos (cdr entry)))
          ;; qualified-id is "namespace-blockid"
          (when (string-match "^\\(.+\\)-\\([^-]+\\)$" qid)
            (let ((ns (match-string 1 qid))
                  (bid (match-string 2 qid)))
              (push (cons bid pos) (gethash ns by-ns))))))
      (maphash
       (lambda (ns entries)
         ;; entries are in buffer order (reversed by push, so reverse again)
         (let* ((ordered (nreverse entries))
                (prev-pos 0))
           (dolist (entry ordered)
             (let ((pos (cdr entry)))
               (when (< pos prev-pos)
                 (push (format "namespace %s: fragment %s at pos %d appears before pos %d (reverse order)"
                               ns (car entry) pos prev-pos)
                       violations))
               (setq prev-pos pos)))))
       by-ns)
      (when violations
...
(defvar agent-shell-invariants--all-checks
  '(agent-shell-invariants--check-process-mark
    agent-shell-invariants--check-fragment-ordering
