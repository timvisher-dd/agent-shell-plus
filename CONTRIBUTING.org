...
Only use =let*= when bindings depend on each other. LLMs tend to default to =let*= even when there are no dependencies between bindings.

For conditional bindings, use =when-let*=. The non-star =when-let= is obsolete as of Emacs 31.1.
