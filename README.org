...
- Desktop notifications when the prompt is idle and waiting for input, riding upstream's =idle= event (so =agent-shell-idle-timeout= tunes the delay, and permission requests notify too) ([[https://github.com/timvisher-dd/agent-shell-plus/pull/2][#2]], [[https://github.com/timvisher-dd/agent-shell-plus/pull/8][#8]])
...
- Upstream merge (agent-shell 0.75.2): dropped the fork's streaming tool-output
  dedup, append-in-place fragment rendering, post-=end_turn= chunk rendering,
  markdown-overlay debounce, process-mark preservation and =gfm-mode= queue
  compose buffer — upstream now implements all of them
