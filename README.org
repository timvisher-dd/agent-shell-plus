...
- Upstream merge (agent-shell 0.75.2): dropped the fork's streaming tool-output
  ...
  compose buffer — upstream now implements all of them.  Desktop idle
  notifications also left the fork: upstream emits an =idle= event and expects
  delivery to live in user config, so the terminal detection, OSC sequences and
  osascript fallback moved to =timvisher-agent-shell-alert.el= in the IDE repo
