...
|                 | agent-shell-queue-compose-cancel                        | Cancel the compose buffer (asks confirmation when modified).                  |
|                 | agent-shell-queue-compose-mode                          | Minor mode for the agent-shell queue-request compose buffer.                  |
|                 | agent-shell-queue-compose-submit                        | Submit (or queue) the contents of the compose buffer.                         |
...
|                 | agent-shell-queue-request                               | Compose (interactive) or send PROMPT (non-interactive) — busy-aware.          |
