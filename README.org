...
Struck-through entries were removed when merging upstream 0.75.2.  The
breadcrumb at the end of each says where the behaviour lives now.
...
- CI workflow and local test runner ([[https://github.com/timvisher-dd/agent-shell-plus/pull/1][#1]], [[https://github.com/timvisher-dd/agent-shell-plus/pull/6][#6]], [[https://github.com/timvisher-dd/agent-shell-plus/pull/8][#8]])
  ...
  - Agent config symlink check (=.agents= entrypoints resolve)
...
- Defensive guards in idle-timer functions so they no-op when shell state is missing instead of erroring ([[https://github.com/timvisher-dd/agent-shell-plus/pull/10][#10]])
- +Streaming tool output with dedup: incremental chunks from codex-acp, batch
  results from claude-agent-acp, =<persisted-output>= stripping and an O(n²)
  rendering fix+ ([[https://github.com/timvisher-dd/agent-shell-plus/pull/7][#7]] → [[https://github.com/xenodium/agent-shell/issues/763][xenodium#763]], [[https://github.com/xenodium/agent-shell/issues/757][xenodium#757]])
- +Append-in-place rendering for streamed fragments, with boundary-newline
  normalization+ ([[https://github.com/timvisher-dd/agent-shell-plus/pull/7][#7]] → [[https://github.com/xenodium/agent-shell/issues/757][xenodium#757]])
- +DWIM context insertion: inserted context lands at the prompt and fragment
  updates no longer drag process-mark past it+ ([[https://github.com/timvisher-dd/agent-shell-plus/pull/7][#7]] → [[https://github.com/xenodium/agent-shell/issues/678][xenodium#678]])
...
- +Render =session/update= chunks streamed after =session/prompt= resolves, so a
  Claude Code Stop-hook bounce-and-regen turn no longer freezes the buffer+
  ([[https://github.com/timvisher-dd/agent-shell-plus/pull/7][#7]] → [[https://github.com/xenodium/agent-shell/issues/678][xenodium#678]])
...
- +Tunable markdown-overlay debounce via =agent-shell-markdown-overlay-debounce-delay=+
  ([[https://github.com/timvisher-dd/agent-shell-plus/pull/7][#7]] → [[https://github.com/xenodium/agent-shell/commit/5a455a4][in-place renderer]])
...
- +=gfm-mode= compose buffer for the interactive =agent-shell-queue-request=+
  ([[https://github.com/timvisher-dd/agent-shell-plus/pull/9][#9]] → [[https://github.com/xenodium/agent-shell/commit/6ae3fdb][agent-shell-prompt-queue.el]])
- +Desktop notifications when the prompt is idle and waiting for input+
  ([[https://github.com/timvisher-dd/agent-shell-plus/pull/2][#2]], [[https://github.com/timvisher-dd/agent-shell-plus/pull/8][#8]] → moved to =timvisher-agent-shell-alert.el= in the IDE repo;
  upstream emits the =idle= event but leaves delivery to the consumer)
...
- Fix streaming cursor drop: a fragment or text entry rendered while the
  cursor sits at the prompt no longer strands point above the inserted text
  - =agent-shell--update-fragment= saves point in an insertion-type t marker
    (its comment already claimed as much) so point rides text inserted at it,
    and snaps to =point-max= whenever point was at end-of-buffer rather than
    only on the late-arrival path.  =shell-maker-with-auto-scroll-edit= can't
    be relied on here: it skips the scroll while =window-end= trails
    =point-max=, then restores point from an insertion-type nil marker
  - =agent-shell--update-text= gets the same treatment; it had no point
    handling at all

Two pieces of [[https://github.com/timvisher-dd/agent-shell-plus/pull/7][#7]] were dropped outright rather than replaced — upstream has no
equivalent, so these are regressions rather than migrations:

- Partial-overlap thought dedup (=agent-shell--thought-chunk-delta=).  Agents
  that re-deliver the full accumulated thought text after incremental tokens,
  such as codex-acp, can render duplicated thought text.
- The =_meta.terminal_output= capability opt-in.  No released acp.el accepts
  =:meta-capabilities=.
...
Steering is always explicit.  =M-x agent-shell-prompt-queue= queues, as it always has.  =M-x agent-shell-prompt-steer= steers the running turn, and errors when the agent cannot steer.  With no turn running there is nothing to steer into, so it submits the prompt instead.  When the agent declines a steer, the running turn is interrupted and the shell says so, rather than letting the agent carry on in a direction you believe you already corrected.
...
| agent-shell-agent-configs                     | The known agent configurations.                                                 |
...
| agent-shell-artist-mode-hook                  | Hook run after entering or leaving ‘agent-shell-artist-mode’.                   |
...
| agent-shell-chat-mode-enabled                 | Whether a new agent shell enables ‘agent-shell-chat-mode’ by default.           |
| agent-shell-chat-mode-hook                    | Hook run after entering or leaving ‘agent-shell-chat-mode’.                     |
| agent-shell-cline-acp-command                 | Command and parameters for the Cline agent client.                              |
| agent-shell-cline-default-model-id            | Default Cline model ID.                                                         |
| agent-shell-cline-default-session-mode-id     | Default Cline session mode ID.                                                  |
...
| agent-shell-codebuddy-acp-command             | Command and parameters for the CodeBuddy agent client.                          |
| agent-shell-codebuddy-environment             | Environment variables for the CodeBuddy agent client.                           |
...
| agent-shell-context-sources                   | Sources to consider when determining C-c C-w automatic context.                 |
| agent-shell-cursor-acp-command                | Command and parameters for the Cursor agent client.                             |
| agent-shell-cursor-authentication             | Configuration for Cursor authentication.                                        |
...
| agent-shell-droid-default-model-id            | Default Droid AI model ID.                                                      |
| agent-shell-droid-default-reasoning-effort    | Default reasoning effort.                                                       |
| agent-shell-droid-default-session-mode-id     | Default Droid AI session mode ID.                                               |
...
| agent-shell-file-display-action               | Display action for files opened from a link, image, or ‘@file’ mention.         |
...
| agent-shell-hermes-acp-command                | Command and parameters for the Hermes agent client.                             |
| agent-shell-hermes-default-session-mode-id    | Default ACP session mode for the Hermes agent.                                  |
| agent-shell-hermes-environment                | Environment variables for the Hermes agent client.                              |
| agent-shell-highlight-blocks                  | Whether or not to highlight source blocks.                                      |
| agent-shell-inhibit-system-sleep              | Non-nil to keep the system awake while an agent is busy.                        |
| agent-shell-kimi-acp-command                  | Command and parameters for the Kimi Code CLI client.                            |
| agent-shell-kimi-default-model-id             | Default Kimi model ID.                                                          |
| agent-shell-kimi-default-session-mode-id      | Default Kimi Code session mode ID.                                              |
| agent-shell-kimi-environment                  | Environment variables for the Kimi Code CLI client.                             |
...
| agent-shell-list-edit-mode-hook               | Hook run after entering or leaving ‘agent-shell-list-edit-mode’.                |
| agent-shell-markdown-render-function          | Function called to render markdown in the current narrowed buffer.              |
...
| agent-shell-mock-agent-acp-command            | Command and parameters for the mock ACP agent.                                  |
| agent-shell-omp-acp-command                   | Command and parameters for the Oh My Pi (omp) ACP client.                       |
| agent-shell-omp-environment                   | Environment variables for the Oh My Pi (omp) client.                            |
...
| agent-shell-prompt-bar-input-mode-hook        | Hook run after entering or leaving ‘agent-shell-prompt-bar-input-mode’.         |
| agent-shell-prompt-bar-mode                   | Non-nil if Agent-Shell-Prompt-Bar mode is enabled.                              |
| agent-shell-prompt-bar-mode-hook              | Hook run after entering or leaving ‘agent-shell-prompt-bar-mode’.               |
...
| agent-shell-session-choices-function          | Function to transform the choices offered when starting a shell.                |
...
| agent-shell-viewport-dismiss-on-send          | Non-nil dismisses the viewport compose window after sending.                    |
| agent-shell-write-inhibit-minor-modes         | List of minor mode commands to inhibit during ‘fs/write_text_file’ edits.       |
| agent-shell-xai-acp-command                   | Command and parameters for the Grok Build ACP client.                           |
...
|                                                       | agent-shell-artist-cancel                               | Discard the scratch art and close the buffer.                                 |
|                                                       | agent-shell-artist-commit                               | Insert scratch art into the calling buffer wrapped in a fence.                |
|                                                       | agent-shell-artist-help                                 | Show all interactive commands for `agent-shell-artist-mode'.                  |
|                                                       | agent-shell-artist-insert-text-drawing                  | Open the scratch artist buffer to draw and insert text from.                  |
|                                                       | agent-shell-artist-mode                                 | Minor mode for `agent-shell' scratch artist buffers.                          |
|                                                       | agent-shell-artist-next-history                         | Show newer art from `agent-shell-artist-history-ring'.                        |
|                                                       | agent-shell-artist-previous-history                     | Show older art from `agent-shell-artist-history-ring'.                        |
|                                                       | agent-shell-auggie-start-agent                          | Start an interactive Auggie agent shell.                                      |
| C-M-u                                                 | agent-shell-backward-up-item                            | Go to the start of the item point navigated into.                             |
|                                                       | agent-shell-chat-mode                                   | Toggle chat-style `Me'/agent labels in the current `agent-shell' buffer.      |
...
|                                                       | agent-shell-codebuddy-start-agent                       | Start an interactive CodeBuddy agent shell.                                   |
|                                                       | agent-shell-completion-mode                             | Toggle agent shell completion with @ or / prefix.                             |
|                                                       | agent-shell-copy-as-markdown                            | Copy the region between BEG and END to the kill ring as markdown.             |
|                                                       | agent-shell-copy-last-output                            | Copy the last agent output to the kill ring.                                  |
|                                                       | agent-shell-copy-link-url-at-point                      | Copy the rendered Markdown link URL at POS (or point) to the kill ring.       |
|                                                       | agent-shell-copy-output                                 | Copy the shell output to the kill ring.                                       |
|                                                       | agent-shell-copy-session-id                             | Copy the current session ID to the kill ring.                                 |
|                                                       | agent-shell-copy-source-block-at-point                  | Copy the rendered fenced code block body at POS (or point) to the kill ring.  |
...
|                                                       | agent-shell-diff-open-file                              | Open the file for the diff at point and jump to the change.                   |
...
|                                                       | agent-shell-fakes-load-session                          | Load and replay a recorded ACP session from TRAFFIC-FILE, popping to it.      |
...
|                                                       | agent-shell-hermes-start-agent                          | Start an interactive Hermes agent shell.                                      |
| -                                                     | agent-shell-image-scale-decrease                        | Narrow the image at point, or every image in the buffer, by one step.         |
| +                                                     | agent-shell-image-scale-increase                        | Widen the image at point, or every image in the buffer, by one step.          |
| 0                                                     | agent-shell-image-scale-reset                           | Reset the image at point, or every image in the buffer, to its rendered size. |
...
|                                                       | agent-shell-kimi-start-agent                            | Start an interactive Kimi Code agent shell.                                   |
|                                                       | agent-shell-kiro-start-agent                            | Start an interactive Kiro agent shell.                                        |
|                                                       | agent-shell-list-edit-dedent-line                       | De-indent the current list item.                                              |
|                                                       | agent-shell-list-edit-indent-line                       | Indent the current list item.                                                 |
|                                                       | agent-shell-list-edit-mode                              | Minor mode for editing markdown-style lists.                                  |
|                                                       | agent-shell-list-edit-newline                           | Insert a newline, continuing the current list item if applicable.             |
|                                                       | agent-shell-markdown-benchmark-compare                  | Run both renderers ITERATIONS times and print a comparison.                   |
|                                                       | agent-shell-markdown-benchmark-stream                   | Benchmark streaming-render with `agent-shell-markdown'.                       |
|                                                       | agent-shell-markdown-benchmark-stream-overlays          | Benchmark streaming-render with `markdown-overlays'.                          |
|                                                       | agent-shell-markdown-image-scale-decrease               | Narrow the image at point, or every image in the buffer, by one step.         |
|                                                       | agent-shell-markdown-image-scale-increase               | Widen the image at point, or every image in the buffer, by one step.          |
|                                                       | agent-shell-markdown-image-scale-reset                  | Drop sizes set by scaling, restoring the ones the images rendered at.         |
|                                                       | agent-shell-markdown-table-next-cell                    | Move point to the start of the next table cell.                               |
|                                                       | agent-shell-markdown-table-previous-cell                | Move point to the start of the previous table cell.                           |
|                                                       | agent-shell-mistral-start-vibe                          | Start an interactive Mistral Vibe agent shell.                                |
|                                                       | agent-shell-mock-agent-start-agent                      | Start an interactive mock ACP agent shell.                                    |
|                                                       | agent-shell-mode                                        | Major mode for agent shell.                                                   |
|                                                       | agent-shell-narrow-to-block                             | Narrow to the last COUNT navigatable blocks in the current buffer.            |
...
|                                                       | agent-shell-omp-start                                   | Start an interactive Oh My Pi (omp) agent shell.                              |
...
|                                                       | agent-shell-prompt-bar-input-mode                       | Minor mode for the `agent-shell' prompt bar input buffer.                     |
|                                                       | agent-shell-prompt-bar-mode                             | Toggle an in-frame prompt bar glued below every `agent-shell' window.         |
|                                                       | agent-shell-prompt-bar-submit                           | Submit the bar's prompt to its shell, or queue it when busy.                  |
...
| C-c C-s                                               | agent-shell-set-session-config-option                   | Set a session config option.                                                  |
...
| C-c C-t                                               | agent-shell-set-session-thought-level                   | Set thought level (reasoning effort) for the current session.                 |
|                                                       | agent-shell-show-usage                                  | Display current session usage information in the minibuffer.                  |
| RET                                                   | agent-shell-submit                                      | Submit the current input to the agent.                                        |
|                                                       | agent-shell-switch-buffer                               | Switch to another `agent-shell-mode' buffer via `completing-read'.            |
...
|                                                       | agent-shell-viewport-compose-send-and-dismiss           | Queue or send the composed prompt, then dismiss the compose window.           |
...
|                                                       | agent-shell-viewport-prompt-queue                       | Queue or immediately send a prompt depending on shell busy state.             |
|                                                       | agent-shell-viewport-prompt-queue-remove                | Remove pending prompts.                                                       |
|                                                       | agent-shell-viewport-prompt-queue-resume                | Resume processing pending prompts in the queue.                               |
...
|                                                       | agent-shell-viewport-set-session-thought-level          | Set thought level (reasoning effort) for the current session.                 |
...
|                                                       | agent-shell-xai-start-grok                              | Start an interactive Grok Build agent shell.                                  |
| C-y or s-v or <paste> or S-<insertchar> or S-<insert> | agent-shell-yank-dwim                                   | Yank or paste clipboard image into `agent-shell'.                             |
* Filing issues
